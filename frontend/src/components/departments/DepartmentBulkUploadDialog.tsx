...
import {
  commitDepartmentBulkUpload,
  downloadDepartmentBulkUploadTemplate,
...
// Upload -> Validate preview -> Commit for Department (Departments
// production-hardening epic, backend Phase 1 / frontend Phase 2) -- a
// sibling of components/locations/LocationBulkUploadDialog.tsx (same
...
// components/departments/ directory rather than reusing components/locations/
// or components/sanctionedStrength/ -- own domain-specific upload flow (own
// row shape, own /departments/bulk-upload/* endpoints), matching this
...
// LocationBulkUploadDialog relies on -- see that component's own docstring
...
// "department-bulk-upload-{id}-errors.xlsx").
...
function PreviewTable({
  ...
  result: DepartmentBulkUploadValidationResponse;
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="max-h-96 overflow-y-auto rounded-md border border-border">
        {result.rows.length === 0 ? (
          ...
          // LocationBulkUploadDialog.tsx's own comment on this exact same
...
export function DepartmentBulkUploadDialog() {
  ...
  const [validationResult, setValidationResult] = useState<DepartmentBulkUploadValidationResponse | null>(null);
  const [commitResult, setCommitResult] = useState<DepartmentBulkUploadCommitResponse | null>(null);
  const [filter, setFilter] = useState<RowStatusFilter>("ALL");
  const [error, setError] = useState<string | null>(null);

  const templateMutation = useMutation({
    mutationFn: downloadDepartmentBulkUploadTemplate,
  });

  const validateMutation = useMutation({
    mutationFn: (f: File) => validateDepartmentBulkUpload(f),
    onSuccess: (data) => {
      setError(null);
  ...
  const commitMutation = useMutation({
    mutationFn: () => commitDepartmentBulkUpload(file as File),
    onSuccess: (data) => {
      setError(null);
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            are matched by Campus Code and Department Code together -- both are required on every row, and a
            changed Department Name for an already-known (campus, code) pair is treated as an update, not a new
            department. See the downloaded template for the exact column layout.
          </DialogDescription>
        </DialogHeader>
          ...
          {commitResult ? (
            <>
              ...
              {commitResult.storage_warning ? (
                ...
                // archival copy failed. See LocationBulkUploadDialog.tsx's
