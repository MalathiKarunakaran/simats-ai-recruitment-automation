...
## Status: Foundation phase (done)

Auth (login, silent session restore via refresh token, logout), a role-based shell (sidebar nav, topbar), dark/light mode, a campus switcher (interactive for global-scope roles, a locked label for single-campus roles), and one real vertical-slice screen: the Executive Dashboard (`GET /dashboard/kpis`), proving auth + role-based rendering + campus-scoped data fetching all work end-to-end against the real backend.

Every other module (Vacancy Requests, Applications, Interviews, Offers, Reports, etc.) is a disabled "soon" nav item — not built yet. This app has no candidate-facing screens; a `CANDIDATE` login shows a clear "not permitted" page (Module 5's candidate portal is separately deferred, same as on the backend).
