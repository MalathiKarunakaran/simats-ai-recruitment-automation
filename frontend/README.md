...
- **Vacancy request action buttons are computed client-side from `(role, status)`**, mirroring `app/services/vacancy_workflow.py`'s own guard clauses exactly rather than just showing every action and letting the backend `409`/`403` sort it out — better UX, verified live at every stage of the approval chain that the right (and only the right) buttons appear. The same "mirror the backend's guard clauses, don't invent stricter or looser client-side rules" approach is used throughout every later module too.
