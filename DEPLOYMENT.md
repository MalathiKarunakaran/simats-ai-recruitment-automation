...
- `PUBLIC_APPLY_BASE_URL` — leave **blank**: the careers pages (`/careers`, `/careers/<slug>`) are served by this frontend, so the apply link and QR code follow `PUBLIC_APP_BASE_URL` / the first CORS origin. Set it only if the careers pages move to a separate domain. (Before 2026-09-07 this held a placeholder `https://careers.simats.edu` that served nothing — remove that line from an older `.env`.)
