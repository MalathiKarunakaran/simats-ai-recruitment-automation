"""Builds the SIMATS Recruitment Tracker workbook template in memory.

This is the template the Import Data page hands out (GET
/migration/tracker-template) and that `tracker_import.py` reads back (POST
/migration/import-tracker-workbook). It is generated on every request from
the enums the importer validates against, so the Master Lists sheet and the
Excel dropdowns can never drift from `CAMPUS_CODES` / `APPLICATION_STATUS_ORDER`.

It used to be a file, `data/SIMATS_Recruitment_Tracker_TEMPLATE.xlsx`,
produced by `scripts/generate_tracker_template.py` and served from disk.
That failed in two ways at once (2026-09-03): the Dockerfile never copied
`data/` into the image, so production answered every download with 404
("Template not found on the server"), and the committed copy was from July
and still listed SHOTS where SHIFT should be. `scripts/generate_tracker_template.py`
now calls `build_workbook` from here and remains only as an ops convenience
for producing the file by hand.

The two sample rows are part of the template on purpose -- they show the
expected formats -- and are highlighted so a person can see they are not
real. `tracker_import` skips them by their fixed IDs (`REQ-2026-001`,
`CAND-0001`) unless the caller passes `include_sample=True`, which the UI
only does when its checkbox is explicitly ticked. Changing those IDs here
without changing them there would silently turn the sample into live data.
"""
