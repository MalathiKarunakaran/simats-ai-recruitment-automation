...
def generate_job_ad(job_posting: JobPosting) -> dict:
    ...
    from app.services.job_postings import default_ad_body  # local: job_postings has no dependency back

    vacancy_request = job_posting.approved_vacancy.vacancy_request
    # The snapshot taken at publish (editable since 2026-09-06) wins; rows
    # that predate it fall back to the request, as before.
    body = job_posting.ad_body or vacancy_request.jd_draft or default_ad_body(vacancy_request)
    return {
        "job_posting_id": job_posting.id,
        "position_title": job_posting.ad_title or vacancy_request.position_title,
